...
SPECS = {
    ...
    "잠행 도적":          {"Res":15,"Sup":5, "Dist":-20,"Log":90,"Agg":45,"Nat":20,"Ord":55,"Shd":65, "Mob":70, "Cpx":85, "Des":50, "Pac":-20},
    ...
    "정기 주술사":        {"Res":20,"Sup":25,"Dist":85, "Log":65,"Agg":55,"Nat":85,"Ord":35,"Shd":20, "Mob":30, "Cpx":60, "Des":65, "Pac":70},
...
SPEC_DESC = {
    ...
    "잠행 도적":          "보이지 않는 곳에서 구조를 붕괴시키는 고수. 모든 것을 꿰뚫어 봅니다.",
    ...
    "정기 주술사":        "대지, 불, 폭풍을 아우르는 원소의 목소리. 거대한 자연의 분노를 다룹니다.",
...
QUESTIONS = [
    {
        "q": "Q1. 모임자리에서 몹시 화가 나는 갈등 상황이 발생했습니다. 당신의 대응은?",
        "opts": [
            {"t": "화가 나지만 침묵을 지키며 화가 가라앉을 때까지 기다린다.",                     "w": {"Res":10, "Ord":6, "Pac":4}}, 
            {"t": "기분은 나쁘지만 분위기를 망치지 않게 조용히 넘긴다.",                           "w": {"Sup":10, "Shd":6, "Pac":4}}, 
            {"t": "대상에게 직설적으로 일침을 날린다.",                                         "w": {"Agg":10, "Des":6, "Mob":4}}, 
            {"t": "저 사람이 왜 저러는지 속으로 분석하며 논리적인 허점을 찾는다.",                  "w": {"Log":10, "Nat":6, "Cpx":4}}
        ]
    },
    {
        "q": "Q2. 새로운 목표나 프로젝트를 진행할 때, 당신의 추진 방식은?",
        "opts": [
            {"t": "결과를 빨리 내기 위해 다소 무리하더라도 공격적으로 추진한다.",                   "w": {"Agg":10, "Log":6, "Mob":4}}, 
            {"t": "남들의 시선을 신경 쓰지 않고 내 직관에 따라 유연하게 움직인다.",                 "w": {"Nat":10, "Dist":6, "Pac":-4}}, 
            {"t": "정해진 절차와 원칙을 준수하며 주변과 조율해 안정적으로 진행한다.",               "w": {"Ord":10, "Sup":6, "Res":4}}, 
            {"t": "모두가 꺼리는 일이라도 목표 달성을 위해 내가 독하게 떠맡는다.",                  "w": {"Shd":10, "Res":6, "Des":4}}
        ]
    },
    {
        "q": "Q3. 회사 업무나 일상에서 전혀 예상치 못한 돌발 사고(중요 데이터 유실, 일정 연쇄 붕괴 등)가 터졌습니다. 가장 당신과 어울리는 것을 고르시오.",
        "opts": [
            {"t": "내 책임을 다한다. 즉각적으로 발로 뛰며 상황에 맞게 수습한다.",                   "w": {"Res":10, "Nat":6, "Mob":4}}, 
            {"t": "당황한 주변 사람들을 챙긴다. 먼저 나서서 서둘러 상황을 진정시킨다.",             "w": {"Sup":10, "Agg":6, "Pac":-4}}, 
            {"t": "한발짝 물러서서 사태를 관망하다가, 눈에 띄지 않게 조용히 처리한다.",             "w": {"Dist":10, "Shd":6, "Pac":4}}, 
            {"t": "시스템과 구조의 오류를 파악하여 원인을 찾고, 복구매뉴얼의 절차를 따라 정확히 복구한다.", "w": {"Log":10, "Ord":6, "Cpx":4}}
        ]
    },
    {
        "q": "Q4. 당신이 상대팀과 겨루는 구기종목 스포츠 선수라면 당신의 플레이스타일은?",
        "opts": [
            {"t": "상대편의 방어를 육탄으로 뚫고 들어가 시원한 공격으로 승부를 본다.",             "w": {"Agg":10, "Des":6, "Mob":4}}, 
            {"t": "순간적인 판단으로 아무도 예상치 못한 플레이를 해낸다.",                          "w": {"Nat":10, "Res":6, "Pac":-4}}, 
            {"t": "미리 준비된 작전의 타이밍을 한 치의 오차도 없이 실행한다.",                      "w": {"Ord":10, "Log":6, "Cpx":4}}, 
            {"t": "몸싸움은 피하고 거리를 유지하며 페이스를 조절하다가, 실수를 이용하여 공격한다.", "w": {"Shd":10, "Dist":6, "Pac":4}}
        ]
    },
    {
        "q": "Q5. 팀플레이 중 극심한 스트레스나 압박을 받는 상황에서, 당신의 모습은?",
        "opts": [
            {"t": "모든 고통을 인내심 있게 받아내며 호전될 때를 기다린다.",                         "w": {"Res":10, "Shd":6, "Pac":4}}, 
            {"t": "루틴이 무너지지 않도록 모두를 독려하며 뛰어다닌다.",                             "w": {"Sup":10, "Ord":6, "Mob":4}}, 
            {"t": "간섭받지 않는 곳에서 나만의 페이스로 자연스럽게 해답을 찾는다.",                 "w": {"Dist":10, "Nat":6, "Pac":4}}, 
            {"t": "복잡하게 얽힌 문제를 가장 빠르게 해결하고자 움직인다.",                          "w": {"Log":10, "Agg":6, "Cpx":4}}
        ]
    },
    {
        "q": "Q6. 까다로운 문제나 상황을 해결할 때, 당신의 주된 전략은?",
        "opts": [
            {"t": "거시적으로 보고 타이밍을 재다가 큰 것 한방으로 상황을 종결시킨다.",             "w": {"Agg":10, "Dist":6, "Des":4}}, 
            {"t": "여러 변수를 복합적으로 고려해 직관적이고 정교한 해답을 낸다.",                   "w": {"Nat":10, "Log":6, "Cpx":4}}, 
            {"t": "원칙을 지키며 흔들림 없이 한 걸음 한 걸음 우직하게 나아간다.",                   "w": {"Ord":10, "Res":6, "Pac":4}}, 
            {"t": "살을 줬으니 뼈를 취한다. 원망을 듣는 것도 감수한다.",                            "w": {"Shd":10, "Sup":6, "Des":4}}
        ]
    },
    {
        "q": "Q7. 조직이 곤경에 처했을 때, 당신이 기여하고자 하는 방향은?",
        "opts": [
            {"t": "책임 지고 앞장서서 돌파구를 연다.",                                          "w": {"Res":10, "Agg":6, "Des":4}}, 
            {"t": "적재적소에 쉴 새 없이 움직이며 동료들의 부족함을 채운다.",                       "w": {"Sup":10, "Nat":6, "Mob":4}}, 
            {"t": "혼란스러운 사람들을 멀리하고 페이스를 유지하며 복잡해진 일을 즐긴다.",           "w": {"Dist":10, "Ord":6, "Cpx":4}}, 
            {"t": "대의를 위해 무엇을 버려야 할지 논리적이고 차갑게 계산한다.",                      "w": {"Log":10, "Shd":6, "Pac":4}}
        ]
    },
    {
        "q": "Q8. 오랫동안 준비한 일이 크게 실패했을 때, 당신의 반응은?",
        "opts": [
            {"t": "실패를 빠르게 털어내고 책임을 인정하며 다시 공격적으로 도전한다.",               "w": {"Agg":10, "Res":6, "Mob":4}}, 
            {"t": "느긋하게 마음을 추스르며 주변 사람들과 함께 위로를 나눈다.",                     "w": {"Nat":10, "Sup":6, "Pac":4}}, 
            {"t": "감정을 배제하고 거리를 두고, 기존 정책의 오류를 찾는다.",                        "w": {"Ord":10, "Dist":6, "Cpx":4}}, 
            {"t": "목표 달성에 필요했던 희생과 논리적 인과관계를 재점검하고 계획을 재설계한다.",    "w": {"Shd":10, "Log":6, "Des":4}}
        ]
    },
    {
        "q": "Q9. 처음 접하는 복잡한 기술이나 업무를 배울 때 당신의 스타일은?",
        "opts": [
            {"t": "정석적인 방법을 신중하게 숙달하며 단단한 기본기를 쌓는다.",                      "w": {"Res":10, "Ord":6, "Pac":4}}, 
            {"t": "팀워크가 중요하므로 진행은 늦더라도 동료들과 서로 돕는다.",                      "w": {"Sup":10, "Shd":6, "Mob":-4}}, 
            {"t": "혼자만의 공간에서 거침없이 이것저것 시도하며 감을 잡는다.",                      "w": {"Dist":10, "Agg":6, "Mob":4}}, 
            {"t": "원리를 파악하고 나만의 노하우를 만들어 효율을 높인다.",                          "w": {"Log":10, "Nat":6, "Cpx":4}}
        ]
    },
    {
        "q": "Q10. 다음 중 가장 마음에 드는 평가는?",
        "opts": [
            {"t": "어떤 압박 속에서도 논리적으로 결과를 만들어내는 에이스.",                        "w": {"Agg":10, "Log":6, "Des":4}}, 
            {"t": "어디서도 1인분 이상! 적재적소 활용되는 만능 해결사.",                            "w": {"Nat":10, "Dist":6, "Mob":4}}, 
            {"t": "느긋하게 조직의 규율과 화합을 다져내는 정신적 지주.",                            "w": {"Ord":10, "Sup":6, "Pac":4}}, 
            {"t": "누군가는 꼭 해야만 했던 위법/편법을 남모르게 처리해낸 다크히어로.",              "w": {"Shd":10, "Res":6, "Cpx":4}}
        ]
    },
    {
        "q": "Q11. 당신의 개인 공간(책상, 방)과 가장 관련이 있는 것은?",
        "opts": [
            {"t": "방해받지 않는 홀로만의 공간을 철벽처럼 유지한다.",                              "w": {"Res":10, "Dist":6, "Pac":4}}, 
            {"t": "목표 달성을 위해 적극적이고 규칙적인 추진력을 준비하는 공간이다.",               "w": {"Agg":10, "Ord":6, "Des":4}}, 
            {"t": "많은 도구들이 갖추어져 있고, 사람들이 편하게 찾아올 수 있도록 한다.",            "w": {"Sup":10, "Log":6, "Pac":4}}, 
            {"t": "누가 뭐래도 상관없다. 나만의 개성을 바탕으로 꾸민다.",                           "w": {"Nat":10, "Shd":6, "Cpx":4}}
        ]
    },
    {
        "q": "Q12. 다음 중 당신과 가장 연관이 있는 것은?",
        "opts": [
            {"t": "규칙은 별로 상관없지만 행동에 대한 책임은 중요하다.",                            "w": {"Dist":10, "Res":6, "Pac":4}}, 
            {"t": "업무 프로토콜을 만들고 공유하면 일의 효율성(속도)이 늘어난다고 생각한다.",       "w": {"Ord":10, "Agg":6, "Mob":4}}, 
            {"t": "자료조사를 통해 얻은 지식과 논리로 동료들에게 더 나은 방향을 제안한다.",         "w": {"Log":10, "Sup":6, "Cpx":4}}, 
            {"t": "내 감과 통찰에 따라 기존의 관행을 갈아엎는 것을 주저하지 않는다.",               "w": {"Shd":10, "Nat":6, "Des":4}}
        ]
    },
    {
        "q": "Q13. 팀플레이 스포츠 중 , 당신이 바라는 이미지는?",
        "opts": [
            {"t": "빠르게 생각을 굴려 타이밍에 맞는 전략으로 상대 에이스의 주의를 완벽히 끈다.",    "w": {"Res":10, "Log":6, "Cpx":4}}, 
            {"t": "나의 희생을 담아 적의 전략을 막는 행동을 한다. 선만 지키면 반칙도 기술이다.",    "w": {"Agg":10, "Shd":6, "Des":4}}, 
            {"t": "나는 매니져다. 벤치에 돌아오는 선수들을 위해 음료와 수건을 준비한다.",           "w": {"Sup":10, "Dist":6, "Pac":4}}, 
            {"t": "경기 규칙을 준수하며 내 역할과 본능에 따라 쉴 새 없이 움직인다.",                "w": {"Nat":10, "Ord":6, "Mob":4}}
        ]
    },
    {
        "q": "Q14. 팀원이 치명적인 실수를 저질러 위기가 찾아왔을 때 당신은?",
        "opts": [
            {"t": "지위와 위치에 관계없이 필요한 곳에서 팀원들의 빈자리를 채운다.",                 "w": {"Dist":10, "Sup":6, "Mob":4}}, 
            {"t": "보이지 않는 큰 흐름를 읽으며 직관적이고 묵직하게 대응한다.",                     "w": {"Ord":10, "Nat":6, "Pac":4}}, 
            {"t": "위기 극복 확률을 극대화하기 위해 복잡하고 철저하게 계산을 한다.",                "w": {"Log":10, "Res":6, "Cpx":4}}, 
            {"t": "내 앞을 막는 문제라면, 압도적인 힘으로 날려버린다. 해결되지 않았다면 힘이 부족했던 건 아닐까?", "w": {"Shd":10, "Agg":6, "Des":4}}
        ]
    },
    {
        "q": "Q15. 갑작스럽게 팀의 리더를 맡게 되었습니다. 당신의 첫 생각은?",
        "opts": [
            {"t": "리더의 짐을 짊어지고 묵직하게 버텨내는 이상적인 리더상을 그리며 다짐한다.",      "w": {"Res":10, "Shd":6, "Pac":4}}, 
            {"t": "가장 빠르고 효율적인 논리적 워크플로를 구상하며 속도를 낸다.",                   "w": {"Agg":10, "Log":6, "Mob":4}}, 
            {"t": "목표를 위해 다그치기보다는 복지규정에 따라 모두가 정당한 대우를 받도록 한다.",   "w": {"Sup":10, "Ord":6, "Pac":4}}, 
            {"t": "복잡한 규정과 권위의식에서 벗어나 나만의 통찰력을 믿고 리드한다.",               "w": {"Nat":10, "Dist":6, "Cpx":4}}
        ]
    },
    {
        "q": "Q16. 오롯이 혼자 보낼 수 있는 자유 시간, 다음 중 어떤 행동을 선호하나요?",
        "opts": [
            {"t": "자연으로 나가 문제로부터 멀리 떨어져 길고 깊은 사색에 잠긴다.",                  "w": {"Dist":10, "Nat":6, "Pac":4}}, 
            {"t": "친구들과 모여 체스나 바둑, 장기같은 보드게임을 즐기며 담소를 나눈다.",           "w": {"Ord":10, "Sup":6, "Cpx":4}}, 
            {"t": "빠른 두뇌 회전과 공격적인 논리 전개와 순발력이 필요한 게임을 한다.",             "w": {"Log":10, "Agg":6, "Mob":4}}, 
            {"t": "전기톱으로 악마를 썰고 인류를 구하는 사명을 가진 둠가이가 되어 스트레스를 날린다.", "w": {"Shd":10, "Res":6, "Des":4}}
        ]
    },
    {
        "q": "Q17. 누군가 당신을 악의적으로 도발해 올 때, 당신의 대처는?",
        "opts": [
            {"t": "방패처럼 묵직하게 감정을 통제하며 오랜 시간 흔들림 없이 버틴다.",               "w": {"Res":10, "Ord":6, "Pac":4}}, 
            {"t": "시야를 빠르게 벗어난 뒤 복잡하고 치명적인 반격을 세팅한다.",                     "w": {"Agg":10, "Dist":6, "Mob":4}}, 
            {"t": "내 살을 깎아서라도 험악해진 분위기를 보듬어 사람들을 안심시킨다.",               "w": {"Sup":10, "Shd":6, "Des":-4}}, 
            {"t": "찰나의 영감을 정교한 논리로 가공하여 상대를 완전히 압도한다.",                   "w": {"Nat":10, "Log":6, "Cpx":4}}
        ]
    },
    {
        "q": "Q18. 거대한 조직이나 회사에서 당신의 능력을 인정해 '단 하나의 특별한 권한(포지션)'을 주겠다고 합니다. 어떤 역할을 고르시겠습니까?",
        "opts": [
            {"t": "조직과 거리를 둔 채 불필요한 낡은 관행을 갈아엎고 경쟁사를 해체해버리는 파괴적인 혁신가", "w": {"Dist":10, "Agg":6, "Des":4}}, 
            {"t": "오랜 시간을 인내하여 외부의 어떤 위기나 공격에도 흔들리지 않도록 조직의 견고한 질서를 확립하는 관리자", "w": {"Ord":10, "Res":6, "Pac":4}}, 
            {"t": "시장의 흐름을 분석하고 복잡한 데이터와 변수들을 긴밀하게 엮어 마스터플랜을 짜는 전략가", "w": {"Log":10, "Nat":6, "Cpx":4}}, 
            {"t": "비밀 프로젝트 팀에 편성되어 분주하게 움직이며 궂은일을 처리해 팀을 돕는 해결사",  "w": {"Shd":10, "Sup":6, "Mob":4}}
        ]
    },
    {
        "q": "Q19. 팀 전체가 진행하던 핵심 프로젝트가 완전히 무산될 절체절명의 위기에 빠졌습니다. 당신의 결단은?",
        "opts": [
            {"t": "모두가 포기하고 떠나더라도, 홀로 자리를 지키며 쏟아지는 비난과 책임을 굳건히 버텨낸다.", "w": {"Res":10, "Dist":6, "Mob":-4}}, 
            {"t": "완벽하고 군더더기 없는 절차로 문제를 정면 돌파하여, 방해물들을 무자비하게 박살 내버린다.", "w": {"Agg":10, "Ord":6, "Des":4}}, 
            {"t": "냉정하게 상황을 계산한 뒤, 가장 가망 있는 팀원이나 대안에 오랜 시간 모은 모든 자원을 몰아준다.", "w": {"Sup":10, "Log":6, "Pac":4}}, 
            {"t": "기존의 금기나 룰을 깨버리고, 직관과 본능에 맡긴 채 복잡하고 변칙적인 방법으로 상황을 반전시킨다.", "w": {"Nat":10, "Shd":6, "Cpx":4}}
        ]
    },
    {
        "q": "Q20. 다음 중 조직내에서 가장 훌륭하다고 생각하는 모습을 고르시오.",
        "opts": [
            {"t": "조급해하지 않고 흔들림 없이 자신의 자리를 묵묵히 지켜낸 전문가.",               "w": {"Dist":10, "Res":6, "Pac":4}}, 
            {"t": "부패한 자들을 쳐내어 조직을 완벽하게 정화해버린 단호한 인사팀.",                 "w": {"Ord":10, "Agg":6, "Mob":4}}, 
            {"t": "복잡한 변수들을 치밀하게 계산하여 팀원 전체의 기량을 최고치로 끌어올린 지휘관.", "w": {"Log":10, "Sup":6, "Cpx":4}}, 
            {"t": "기존의 낡은 생태계를 가차 없이 파괴해버렸지만, 결과적으로 뼈아픈 혁신을 이뤄낸 개척자.", "w": {"Shd":10, "Nat":6, "Des":4}}
        ]
    },
    {
        "q": "Q21. 업무를 수행할 때, 다음 중 당신이 가장 좋아하는 장면은?",
        "opts": [
            {"t": "쉴 새 없이 현장을 누비며 업무 장애물을 폭발적인 추진력으로 박살냄.",            "w": {"Mob":10, "Des":6, "Agg":4}}, 
            {"t": "긴 호흡 속에서 난잡한 자원과 변수들을 완벽하게 계산하고 설계해냄.",             "w": {"Cpx":10, "Pac":6, "Log":4}}, 
            {"t": "뼈를 깎는 고통을 감수하고 경쟁자를 제끼고 압도적인 실적을 쟁취했을 때.",         "w": {"Des":10, "Res":6, "Shd":4}}, 
            {"t": "조직 내 정치판에서 벗어나 나만의 질서 있는 루틴을 느긋하게 돌리며 성과를 냄.",   "w": {"Pac":10, "Dist":6, "Ord":4}}
        ]
    },
    {
        "q": "Q22. 동료들과 하나의 팀으로 일하는 과정에서, 당신이 가장 빛을 발하는 순간은?",
        "opts": [
            {"t": "어디선가 문제가 생기면 직관적으로 가장 먼저 뛰어가, 발 빠르게 동료의 빈틈을 메워줄 때.", "w": {"Mob":10, "Sup":6, "Nat":4}}, 
            {"t": "모두와 거리를 둔 채, 남들은 이해하기 힘든 복잡하고 고독한 시스템 구축을 완벽히 끝냈을 때.", "w": {"Cpx":10, "Dist":6, "Log":4}}, 
            {"t": "경쟁사를 완전히 무너뜨리겠다는 일념 하나로, 보이지 않는 곳에서 집요하게 치명적인 공격을 준비할 때.", "w": {"Des":10, "Agg":6, "Shd":4}}, 
            {"t": "긴 호흡으로 묵묵히 외부의 압박을 막아내며, 팀의 원칙과 멘탈이 무너지지 않도록 든든하게 지켜줄 때.", "w": {"Pac":10, "Res":6, "Ord":4}}
        ]
    },
    {
        "q": "Q23. 다음 중 가장 마음에 드는 감정은?",
        "opts": [
            {"t": "나 홀로 바람처럼 세상을 떠도는 자유로움",                                    "w": {"Mob":10, "Nat":6, "Dist":4}}, 
            {"t": "지적 한계를 시험하는 치열한 두뇌 싸움의 긴장감",                               "w": {"Cpx":10, "Ord":6, "Log":4}}, 
            {"t": "눈앞의 적을 부숴버리는 광기 어린 돌파의 시원함",                               "w": {"Des":10, "Shd":6, "Agg":4}}, 
            {"t": "인내, 희생과 헌신으로 동료와 동고동락하는 즐거움",                             "w": {"Pac":10, "Sup":6, "Res":4}}
        ]
    },
    {
        "q": "Q24. 혼돈의 도가니, 긴급한 위기 상황에서 당신을 지배하는 감각은?",
        "opts": [
            {"t": "머리보다 몸이 먼저 반응하여 문제를 향해 정면돌파.",                            "w": {"Mob":10, "Agg":6, "Nat":4}}, 
            {"t": "쏟아지는 압박을 견디면서 머릿속으로 다음 수를 설계.",                          "w": {"Cpx":10, "Res":6, "Log":4}}, 
            {"t": "타이밍을 지켜보다가 영리한 술수로 상황 역전.",                                 "w": {"Des":10, "Dist":6, "Shd":4}}, 
            {"t": "질서를 통해 모두가 만족할 평화로운 해결책을 제안.",                            "w": {"Pac":10, "Ord":6, "Sup":4}}
        ]
    }
]
...
def radar_svg(u_vec, spec_vec):
    ...
    return f"""<svg viewBox="0 0 400 450" xmlns="http://www.w3.org/2000/svg" style="background:#0f172a;border-radius:12px;">
  {grid}{axes}
  ...
  {labels}
  <rect x="55"  y="415" width="12" height="12" fill="#6366f1" rx="2"/>
  <text x="73"  y="426" fill="#94a3b8" font-size="11" font-family="sans-serif">나의 성향</text>
  <rect x="175" y="415" width="12" height="12" fill="#f59e0b" rx="2"/>
  <text x="193" y="426" fill="#94a3b8" font-size="11" font-family="sans-serif">추천 전문화</text>
</svg>"""
